...
import {
  AlertHistoryViewMode,
  AlertHistoryWithRelations,
...
const tabLabels: Record<AlertHistoryViewMode, string> = {
        ...
        <div className="flex gap-2 lg:items-center justify-between mb-8 lg:flex-row flex-col-reverse">
          <div>
            ...
            <p className="text-gray-600">
              Suivi de toutes les opérations effectuées sur Alert #
              {selectedItem?.alert?.code}
            </p>
          </div>
          <button
            onClick={() => router.back()}
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="border-b border-gray-200">
            <nav className="flex -mb-px">
              {(["timeline", "table", "changes"] as AlertHistoryViewMode[]).map(
                (tab) => (
                  <button
                    key={tab}
